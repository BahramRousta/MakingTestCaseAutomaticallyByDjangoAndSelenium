...
class RunTestCaseAPIView(APIView):
    ...
    def put(self, request):
        ...
        if serializer.is_valid(raise_exception=True):
            test_scenario_id = serializer.validated_data['test_scenario']
            test_scenario = TestScenario.objects.filter(id=test_scenario_id).first()
            if test_scenario:
                test_case = TestCase.objects.filter(title=serializer.validated_data['title'],
                                                    test_scenario=test_scenario).first()
                serializer.update(instance=test_case, validated_data=serializer.validated_data)
            return Response(status=200)
        else:
            return Response(status=400, data=serializer.errors)
